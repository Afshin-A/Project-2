...
//Used in functions main and dataInput. Determines conditions for while loops in aforementioned functions.
bool continueLoop(string prompt);
//Takes input, performs checks, and sets up fields in a InterestPlan object
void dataInput(InterestPlan* plan);

int main() {
	cout << "Welcome to the Airgead Banking App" << endl;
	cout << endl;
	system("pause"); //From cstlib. halts program until user pushes any key
	cout << endl;
...
void dataInput(InterestPlan* plan) {
	...
	//if the loop reaches this point, all values satisfy the conditions set.
